...
- [x] INT-1 — End-to-End Trace CLI / Operator Report. _Delivered 2026-06-19; extends `crates/cognitive-demo` with the `cognitive-demo` binary so a human can inspect the INT-0 trace without reading Rust structs or test output. Doctrine: **Reading verifies. Hypothesis proposes. Probe queue classifies. Governance reviews. Execution intent records. Observation quarantines. Promotion refuses. Nothing becomes evidence. Nothing trains.** It adds NO new authority and NO new cognition — a thin operator surface over the existing canonical trace. Three commands: `cognitive-demo trace [--out PATH]` writes the canonical `CognitiveTrace` JSON; `cognitive-demo report --trace PATH [--out PATH]` renders a plain operator report; `cognitive-demo replay --trace PATH` confirms a byte-identical reproduction. **Trust-boundary design (the load-bearing INT-1 property):** because `CognitiveTrace` is `Serialize` but NOT `Deserialize`, `report`/`replay` do NOT parse a provided file back into authority — the new pure `verify_trace_json(provided)` RE-DERIVES the canonical trace via `CognitiveTrace::demo()` and compares the provided string BYTE-FOR-BYTE, returning `TraceError::TraceMismatch` on any difference; the report is then rendered from the RE-DERIVED canonical trace (`CognitiveTrace::to_report()` reads the trace's own private fields), so a tampered/stale/foreign `trace.json` can never be laundered into a clean report or a passing replay (both refuse it). The report shows all seven stages (reading → hypothesis → probe queue → review → intent → observation → promotion) with the ids/hashes needed to audit/replay, prints all nine `BOUNDARY_LINES` verbatim, and states explicitly that nothing executed, nothing became evidence, and training stayed false (`training_justified=false`); `to_report` is pure FORMATTING — it computes no new verdict, calls no frozen API, and returns no authority object, so the report can never disagree with or be more permissive than the trace it describes. New pure library surface (`run_trace`/`run_report`/`run_replay`/`verify_trace_json`/`to_report`/`BOUNDARY_LINES`, all string-in/string-out); `std::fs` lives ONLY in the new `src/main.rs` (the binary is a thin I/O shell), so the trace CORE (lib.rs) and the example stay filesystem-free and the trace RESULT can never depend on disk. 8 new tests (the rubric's 8 first-tests) → 20 unit total (12 INT-0 + 8 INT-1), fmt + clippy clean. release_check INT-1 gate: CLI-core + report signals + the trust-boundary greps (`verify_trace_json` re-derives via `CognitiveTrace::demo()`, `TraceMismatch` refuses) + all 8 INT-1 test-name pins + the unit-count pin raised 12→20 + a relaxed-but-stronger purity model (executor/network banned EVERYWHERE in src + example; clock/entropy banned in src; floats banned; and a NEW fs-confined scan asserting `std::fs`/`File`/`fs::write`/`fs::read`/`OpenOptions` appear in NEITHER lib.rs NOR the example — only `main.rs`) + an end-to-end BINARY smoke (build the CLI, `trace --out` twice → `cmp -s` byte-identical determinism + the canonical markers present, `report --trace --out` → all 7 stages + all 9 boundary lines + the explicit refusals + never an `executed`/`promoted`/`granted` status, `replay` accepts the canonical trace, and a TAMPERED trace is REFUSED by BOTH replay and report). Three live sabotage probes each restored byte-identical (md5, never `git checkout`): (a) defeat the `verify_trace_json` comparison so a tampered trace is accepted → caught by the tamper-rejection unit tests (exit 101) AND the binary smoke; (b) leak `std::fs::read` into the pure lib core → caught SOLELY by the fs-confined scan (compiled, tests/clippy clean, isolated); (c) drift a boundary line (`Nothing trains.`→`Nothing learns.`) → caught by `report_contains_all_boundary_lines` AND the binary smoke's verbatim boundary grep. A read-only adversarial panel (Workflow, 4 Explore lenses, refute-by-default, no-compile-to-disk, driving the compiled binary under `mktemp`: trust-boundary/tamper-laundering, fs-confinement/purity/no-execution, report-fidelity/no-authority/overclaim, scope/no-frozen-edit/gate-vacuity) → **0 REAL findings, fully dry on the first round, no debris** (each lens independently confirmed tampered/forged traces are refused by both replay and report). Purely additive: only `crates/cognitive-demo/{Cargo.toml,src/lib.rs}` + the new `src/main.rs` + the gate block; NO frozen crate source touched, tags unmoved (`reading-track-v0.1`@`f6fa55a`, `hypothesis-track-v0.1`@`bb20acf`), P12 `training_justified=false`, P13–P15 closed; no new dependency (no Cargo.lock change). No model, no training, no execution, no promotion. Gate green + byte-silent (0/0B/0B)._
- [x] INT-2 — Trace Question Harness / Operator Interrogation Surface. _Delivered 2026-06-19; extends `crates/cognitive-demo` (the `cognitive-demo` binary) with a deterministic, FINITE, enum-backed audit-question surface over the INT-0/INT-1 canonical trace so an operator can ask what happened, what did NOT, and why authority was refused — with NO LLM, NO natural-language parser, and NO new authority. Doctrine: **Trace questions explain the trace. They do not create authority. They do not execute. They do not promote. They do not train.** Two commands: `cognitive-demo questions` lists the closed set; `cognitive-demo ask --trace PATH --question SLUG [--out PATH]` answers ONE enumerated question. The eight slugs: `what-read`, `what-was-proven`, `what-was-hypothesized`, `what-probe-was-requested`, `was-anything-executed`, `did-anything-become-evidence`, `why-was-promotion-refused`, `did-training-open`. **Closed by construction:** a question is a `TraceQuestion` enum variant (`ALL: [TraceQuestion; 8]`); `TraceQuestion::from_slug` does EXACT-match only (no fuzzy/prefix/case/trim), returning `None` on any miss; `run_ask` maps `None` → `TraceError::UnknownQuestion` and FAILS CLOSED before consulting any trace — there is no free-form path. **Trust boundary (INT-1's, applied to `ask`):** `run_ask` answers ONLY the trace returned by `verify_trace_json`, which RE-DERIVES the canonical trace via the pure `CognitiveTrace::demo()` and byte-compares the provided file, refusing any tampered/stale/foreign input (`TraceError::TraceMismatch`) BEFORE answering — `CognitiveTrace` stays `Serialize` but NOT `Deserialize`, so a provided trace is never parsed back into authority (a tampered trace is refused for every question). **Answers are not authority:** the private `CognitiveTrace::answer` + eight `answer_*` renderers FORMAT only the trace's already-recorded private fields (no new verdict, no frozen API, no authority object), distinguish the stages (`what-was-proven` says exactly the reading receipt was verified and the hypothesis only PROPOSES / is not a claim; `was-anything-executed` answers No with `requires_operator`, never `executed`; `did-anything-become-evidence` answers No with `rejected` + `grants_promotion=false`; `why-was-promotion-refused` says the promotion did not occur; `did-training-open` says training stayed false), include the relevant ids/hashes, and end with the five-line INT-2 boundary (`ASK_BOUNDARY_LINES`). Purity is unchanged: the whole surface is in the pure library; the only filesystem access is the pre-existing `main.rs` I/O shell, so no new dependency, no new file, no Cargo.toml change. 12 new tests (the rubric's 10 first-tests + `ask_answer_preserves_authority_boundary` + `ask_answer_is_not_authority`) → 32 unit total (12 INT-0 + 8 INT-1 + 12 INT-2), fmt + clippy clean. release_check INT-2 gate: surface signals + the fail-closed/re-derive pins (`TraceQuestion::from_slug`, `verify_trace_json`, `UnknownQuestion`) + all 12 INT-2 test-name pins + the unit-count pin raised 20→32 + an end-to-end BINARY smoke (`questions` lists all 8 slugs; `ask what-read` carries the real `reading_answer_hash` and all five boundary lines verbatim; the execution/evidence/promotion/training answers say No/refuse; a NO-AUTHORITY guard asserts no answer line ends in an affirmative `executed`/`promoted`/`granted`/`recorded` status and no grant reads `true`; an UNKNOWN question and a TAMPERED trace are BOTH refused, exit non-zero). One self-found gap folded BEFORE sabotage: the boundary smoke originally pinned only two of the five lines and the test only lines [0]/[4], so a drift in a middle line (`create authority`/`execute`/`promote`) could slip both — closed by a verbatim five-line gate loop + all five literals pinned in the test. Three live sabotage probes each restored byte-identical (md5, never `git checkout`): (a) fail-OPEN the ask tamper-refusal (answer a tampered trace with canonical content) → caught by `ask_refuses_tampered_trace` (exit 101) AND the binary smoke; (b) fail-OPEN an unknown question (default to a variant) → caught by `ask_refuses_unknown_question` (exit 101) AND the binary smoke; (c) a COORDINATED boundary drift (`They do not promote.`→`They may promote.` in BOTH the const AND its test literal) → the crate test suite stayed GREEN (32/0) yet the gate still failed (exit 1), caught SOLELY by the new verbatim five-line boundary loop — proving the hardening is load-bearing even against a matching test edit. A read-only adversarial panel (Workflow, 4 Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under `mktemp`: fail-closed/closure, trust-boundary/tamper-laundering, answer-fidelity/no-authority/overclaim, scope/no-frozen-edit/purity/gate-vacuity) → **0 REAL findings, fully dry on the first round, no debris** (every lens drove the binary: 7–8 non-slug questions refused, 4–6 tampered/foreign traces refused before answering, all 8 answers carrying the canonical hashes + five boundary lines + no affirmative status). Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` + the gate block; NO Cargo.toml change, NO new file, NO new dependency; no frozen crate source touched, tags unmoved (`reading-track-v0.1`@`f6fa55a`, `hypothesis-track-v0.1`@`bb20acf`), P12 `training_justified=false`, P13–P15 closed. No LLM, no natural-language parser, no training, no execution, no promotion. Gate green + byte-silent (0/0B/0B)._
- [x] INT-3 — Prototype Demo Bundle / Operator Repro Pack. _Delivered 2026-06-19; extends `crates/cognitive-demo` (the `cognitive-demo` binary) with one reproducible operator pack over the INT-0/INT-1/INT-2 canonical trace, plus a re-deriving verifier. Doctrine: **The bundle demonstrates the prototype. It does not create evidence. It does not create authority. It does not execute. It does not promote. It does not train.** Two commands: `cognitive-demo bundle --out DIR` writes the pack; `cognitive-demo bundle-verify --path DIR` re-derives and checks it. The pack is four files PURELY derived from the canonical trace: `trace.json` (the machine record, `run_trace`), `report.txt` (the operator report, `run_report`), `questions.txt` (the full audit transcript: `list_questions` menu + every enumerated question answered via `run_ask`, by `run_questions_doc`), and `manifest.json` (a `BundleManifest` that hashes every CONTENT file, records a replay proof + the six-line boundary). **Trust boundary (re-derive, never trust the files):** `verify_bundle` calls `canonical_bundle()` (which builds from `run_trace`/`CognitiveTrace::demo()`) and byte-compares each provided file — it does NOT parse/deserialize any file and does NOT check the manifest's own recorded hash against the file. A missing file is `TraceError::BundleMissingFile`; any tampered/stale/foreign file (INCLUDING the manifest) is `TraceError::BundleMismatch`. `CognitiveTrace` AND `BundleManifest`/`BundleFileEntry`/`BundleReplayProof` derive `Serialize` but NOT `Deserialize`, so no bundle file is ever read back into authority — a tampered bundle can never pass. **Manifest integrity:** `bundle_content_hash` is Rust's `DefaultHasher` (deterministic, dependency-free), named honestly as `rust-default-hasher-u64-hex` (NOT a crypto digest); the manifest hashes the three content files with distinct content-dependent hashes and does NOT hash itself (no fixpoint); the load-bearing integrity check is the full byte-for-byte re-derivation, of which the hash is a demonstrable part. **Purity / fs-confinement:** the bundle is a pure function of fixed inputs (two bundles are byte-identical, file for file); `std::fs` lives ONLY in `src/main.rs` (`write_bundle`/`read_bundle`), so the bundle CONTENT can never depend on disk — the lib that derives/verifies it stays filesystem-free. **Not authority:** no bundle file shows an affirmative `executed`/`promoted`/`granted`/`recorded` status or a true grant; the trace records `training_justified=false`; the P12 verdict and verifier receipt are unmoved. 12 new tests (the rubric's 10 first-tests + `bundle_verify_rejects_tampered_manifest` + `bundle_output_is_not_authority`) → 44 unit total (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3), fmt + clippy clean. release_check INT-3 gate: surface signals + the re-derive pin (`let canonical = canonical_bundle()`) + all 12 INT-3 test-name pins + the unit-count pin raised 32→44 + an end-to-end BINARY smoke (the four files present; the manifest names + hashes every content file; the six boundary lines verbatim; two bundles byte-identical file-for-file; a NO-AUTHORITY status guard; `bundle-verify` accepts the pristine pack and REFUSES a tamper of EACH file (trace/report/questions/manifest) + a missing file + a foreign bundle). One self-found gap folded BEFORE sabotage: `bundle_manifest_hashes_all_files` recomputes hashes with the same `bundle_content_hash`, so a constant/fake hash would slip both it and the count check — closed by a gate check that the content hashes are DISTINCT (content-dependent). Three live sabotage probes each restored byte-identical (md5, never `git checkout`): (a) make `verify_bundle` TRUST the files (skip the byte-comparison) → caught by all four tamper tests (exit 101) AND the binary smoke; (b) a constant fake `bundle_content_hash` → the crate suite stayed GREEN (44/0, the hash test is self-referential) yet the gate failed (exit 1), caught SOLELY by the new distinct-hash check; (c) a COORDINATED boundary drift (`It does not promote.`→`It may promote.` in BOTH the const AND its test literal) → suite GREEN (44/0) yet gate exit 1, caught SOLELY by the binary smoke's verbatim six-line manifest loop — (b) and (c) prove the hardening is load-bearing even against a matching test edit. A read-only adversarial panel (Workflow, 4 Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under `mktemp`: re-derivation/tamper-rejection, manifest-integrity/no-self-trust, output-not-authority/overclaim, scope/purity/fs-confinement/gate-vacuity) → **0 REAL findings, fully dry on the first round, no debris** (every lens drove the binary: tampered trace/report/questions/manifest + missing + foreign all refused, pristine verifies, two bundles byte-identical, no affirmative status). Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` + the gate block; NO Cargo.toml change, NO new file, NO new dependency; no frozen crate source touched, tags unmoved (`reading-track-v0.1`@`f6fa55a`, `hypothesis-track-v0.1`@`bb20acf`), P12 `training_justified=false`, P13–P15 closed. No LLM, no natural-language parser, no training, no execution, no evidence promotion. Gate green + byte-silent (0/0B/0B)._
- [x] MTRACE-0 — Multi-Trace Scenario Pack / Variation Without Authority Expansion. _Delivered 2026-06-19; extends `crates/cognitive-demo` (the `cognitive-demo` binary) with a small deterministic scenario pack that produces MULTIPLE `CognitiveTrace` bundles, each proving the SAME authority boundary under a DIFFERENT review/observation/promotion outcome. Doctrine: **Scenarios vary the path. They do not vary the authority. Nothing executes. Nothing becomes evidence. Nothing promotes. Nothing trains.** Three commands: `cognitive-demo scenarios` lists the set; `cognitive-demo scenario-pack --out DIR` writes one bundle subdirectory per scenario + a `pack-manifest.json`; `cognitive-demo scenario-verify --path DIR` re-derives and checks the whole pack. **Four scenarios, deterministic outcomes of the FROZEN chain:** `happy-boundary` (governance approves a low-risk/reversible queued probe → intent `requires_operator`, observation `requires_review`, promotion `rejected` — THIS IS the frozen canonical `demo()` trace, byte-for-byte); `review-rejected` (governance rejects a queued probe → intent `blocked`, observation `rejected`); `review-deferred` (governance defers → intent `blocked`, observation `rejected`); `high-risk-blocked` (the probe is classified `blocked` because it is high-risk AND irreversible → there is NO approval path, since the frozen layer refuses to approve a blocked probe, so nothing executes). (`tampered-input` from the rubric is the cross-cutting verify-refusal property, tested per scenario, not a path variant.) **Variation without authority expansion:** a `Scenario` enum varies ONLY the probe risk/reversibility and the governance decision (`Scenario::risk`/`reversibility`/`review_decision` passed into the new `CognitiveTrace::build_scenario`, which `build()` now delegates to with `HappyBoundary`); everything else — the reading verification, the receipt citation, the chain linkage, and the verdict computation — is identical and read from the frozen crates. EVERY scenario preserves the full boundary: execution never `executed` (`nothing_executed`), observation never `recorded` and `observation_only` (`observation_quarantined`), promotion `rejected` with `grants_promotion=false` (`promotion_refused`/`nothing_becomes_evidence`), and `training_justified=false` (P12 unmoved, verifier receipt byte-identical). A rejected/deferred review yields a `blocked` (never executable) intent; a blocked probe has no approval path. **Re-derive, never trust:** `scenario_bundle`/`scenario_pack_manifest` are PURELY derived; `verify_scenario_bundle`/`verify_scenario_pack_manifest` RE-DERIVE the canonical scenario bundle / pack manifest and byte-compare (via the shared `compare_bundle` core) — `CognitiveTrace`, `BundleManifest`, AND `ScenarioPackManifest` all derive `Serialize` but NOT `Deserialize`, so no file is parsed back into authority and a tampered/missing/foreign scenario is refused. **Freeze preserved:** the refactor that parameterized `build()` and made `canonical_bundle`/`run_questions_doc`/`verify_bundle` delegate to shared cores (`trace_bundle`/`questions_doc`/`compare_bundle`/`bundle_manifest_with`) left the integration-demo-v0.1 canonical trace and bundle byte-for-byte identical (all 44 frozen tests pass; happy-boundary == `demo()`). 12 new tests (the rubric's 10 first-tests + `happy_boundary_scenario_equals_canonical_demo` + `scenarios_are_distinguishable`) → 56 unit total (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3 + 12 MTRACE-0), fmt + clippy clean. release_check MTRACE-0 gate: surface signals + the re-derive pins (`compare_bundle(&scenario_bundle(scenario)?`, `provided == scenario_pack_manifest()?`) + all 12 MTRACE-0 test-name pins + the unit-count pin raised 44→56 + a BINARY smoke (`scenarios` lists 4; `scenario-pack` writes 4 subdirs × 4 files + `pack-manifest.json`; two packs byte-identical file-for-file; scenarios distinguishable by status — `requires_operator` for happy, `blocked` for the reviews, `blocked` probe for high-risk; a NO-AUTHORITY guard over the WHOLE pack; `scenario-verify` accepts the pristine pack and REFUSES a tampered scenario trace / scenario manifest / pack-manifest + a missing file + a foreign scenario). One self-found freeze-preservation gap folded BEFORE sabotage: `happy_boundary_scenario_equals_canonical_demo` compares `scenario_trace(Happy)` to `demo()` — but both use the SAME parameterized builder, so a silent drift of the happy-boundary risk/reversibility (path unchanged) would slip it AND the status greps — closed by pinning the frozen canonical `hypothesis_id` (`16880898425785712701`, a stable FNV id from the frozen hypothesis-layer) in the gate. Three live sabotage probes each restored byte-identical (md5, never `git checkout`): (a) make a rejected review APPROVE (a rejected review producing executable intent) → caught by `review_rejected_scenario_blocks_intent` (exit 101) AND the binary smoke; (b) make `verify_scenario_bundle` TRUST the files (skip the byte-comparison) → caught by `tampered_scenario_bundle_is_refused` (exit 101) AND the smoke; (c) a silent happy-boundary canonical DRIFT (`Scenario::risk(HappyBoundary)` 100→200, path unchanged) → the crate suite stayed GREEN (56/0) yet the gate failed (exit 1), caught SOLELY by the new `hypothesis_id` freeze-pin — proving the freeze-preservation pin is load-bearing. A read-only adversarial panel (Workflow, 4 Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under `mktemp`: scenario-correctness/boundary-preservation, freeze-preservation/happy==canonical, re-derivation/tamper-rejection, scope/purity/gate-vacuity) → **0 REAL findings, fully dry on the first round, no debris** (every lens drove the binary: all four scenarios hold the boundary, happy-boundary is byte-identical to the canonical trace, all five tamper/missing/foreign cases refused, the pack is deterministic). Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` + the gate block; NO Cargo.toml change, NO new file, NO new dependency; no frozen crate source touched, tags unmoved (`reading-track-v0.1`@`f6fa55a`, `hypothesis-track-v0.1`@`bb20acf`, `integration-demo-v0.1`@`95b586d`), P12 `training_justified=false`, P13–P15 closed. No LLM, no training, no execution, no evidence promotion, no changes to frozen crates. Gate green + byte-silent (0/0B/0B)._
- [x] MTRACE-1 — Scenario Matrix / Boundary Coverage Report. _Delivered 2026-06-20; extends `crates/cognitive-demo` (the `cognitive-demo` binary) with a deterministic boundary-coverage MATRIX over the MTRACE-0 scenario pack — proving, at a glance, that every scenario covers its expected path variation while preserving the SAME authority boundaries. Doctrine: **The matrix summarizes coverage. It does not create authority. It does not execute. It does not promote. It does not train.** Three commands: `cognitive-demo scenario-matrix --pack DIR --out matrix.json` emits the canonical coverage matrix; `cognitive-demo scenario-matrix-report --matrix matrix.json --out matrix.txt` renders a plain report; `cognitive-demo scenario-matrix-verify --pack DIR --matrix matrix.json` re-derives and checks both. **The matrix:** one `MatrixRow` per scenario records its PATH (review/probe/intent/observation/promotion status + the `training_not_justified` verdict) AND the four BOUNDARY cells `no_execution`/`no_evidence`/`no_promotion`/`no_training` (all `true` for every scenario), plus a `MatrixCoverage` summary (4 scenarios × 4 boundaries = 16 cells, `cells_proven=16`, `all_boundaries_hold=true`, and the DISTINCT review/intent/probe statuses proving the variation is real). **Proves, not asserts:** every boundary cell is the trace's REAL verdict (`no_execution=trace.nothing_executed()`, `no_evidence=trace.nothing_becomes_evidence()`, `no_promotion=trace.promotion_refused()`, `no_training=!trace.training_justified()`), and each status row matches the scenario's trace exactly. **Re-derive, never trust:** the matrix is PURELY re-derived from `Scenario::ALL` (it never trusts the pack files); `scenario-matrix --pack` first VERIFIES the pack (via the new pure `verify_scenario_pack`, which re-derives every scenario bundle + the pack manifest) and refuses a tampered pack BEFORE emitting; `verify_scenario_matrix`/`scenario_matrix_report` RE-DERIVE the canonical matrix and byte-compare the provided JSON (`TraceError::MatrixMismatch` on any difference), and the report renders from the RE-DERIVED canonical struct, never the provided file. `ScenarioMatrix`/`MatrixRow`/`MatrixCoverage` derive `Serialize` but NOT `Deserialize`, so a provided matrix is never parsed into authority — a tampered matrix OR a tampered pack is refused by verify, report, AND emit. **Not authority:** no matrix/report field shows an affirmative `executed`/`promoted`/`granted`/`recorded` status, a true grant, or a `training_justified` verdict; the report is pure FORMATTING of the re-derived matrix (no new verdict). The frozen canonical trace is unchanged (happy-boundary still == `demo()`). `scenario-verify` was refactored to use the shared `verify_pack_at`→`verify_scenario_pack` core (behaviour preserved). 12 new tests (the rubric's 10 first-tests + `scenario_matrix_distinguishes_all_paths` + `scenario_matrix_report_is_not_authority`) → 68 unit total (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3 + 12 MTRACE-0 + 12 MTRACE-1), fmt + clippy clean. release_check MTRACE-1 gate: surface signals + the re-derive pin (`provided == scenario_matrix()?`) + all 12 MTRACE-1 test-name pins + the unit-count pin raised 56→68 + a BINARY smoke (the matrix records every scenario + all status fields + the boundary cells; `cells_proven=16`, `all_boundaries_hold=true`; no false boundary cell or `training_justified`; distinguishable paths; a NO-AUTHORITY guard; two matrices byte-identical; the report carries the coverage + the five boundary lines verbatim + the explicit no-execution prose; `scenario-matrix-verify` accepts the pristine pack+matrix and REFUSES a tampered matrix by verify AND report + a tampered pack by emit AND verify). Three live sabotage probes each restored byte-identical (md5, never `git checkout`): (a) make `verify_scenario_matrix` TRUST the provided matrix (skip the byte-comparison) → caught by `scenario_matrix_verify_rejects_tampered_matrix` (exit 101) AND the binary smoke; (b) make `verify_scenario_pack` SKIP the per-scenario bundle verification (trust a tampered pack) → caught by `scenario_matrix_verify_rejects_tampered_pack` (exit 101) AND the smoke; (c) a matrix boundary-line DRIFT (`The matrix summarizes coverage.`→ an authority-weakening claim) → the crate suite stayed GREEN (68/0, the report-boundary test iterates the const) yet the gate failed (exit 1), caught SOLELY by the binary smoke's verbatim report-boundary loop — proving the boundary pin is load-bearing. A read-only adversarial panel (Workflow, 4 Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under `mktemp`: matrix-coverage/fidelity, re-derivation/tamper-rejection, output-not-authority/no-overclaim, scope/purity/freeze/gate-vacuity) → **0 REAL findings, fully dry, no debris** (the first attempt was abandoned mid-run by a session-usage limit — absence of verification, NOT a pass — and the panel was re-run to a genuine dry round; every lens drove the binary and confirmed each row matches its trace, the boundaries all hold, tampered matrix/pack are refused by verify/report/emit, and the canonical trace is byte-identical). Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` + the gate block; NO Cargo.toml change, NO new file, NO new dependency; no frozen crate source touched, tags unmoved (`reading-track-v0.1`@`f6fa55a`, `hypothesis-track-v0.1`@`bb20acf`, `integration-demo-v0.1`@`95b586d`), P12 `training_justified=false`, P13–P15 closed. No LLM, no training, no execution, no evidence promotion, no frozen crate edits. Gate green + byte-silent (0/0B/0B)._
- [x] MTRACE-2 — Scenario Failure Injection / Boundary Regression Pack. _Delivered 2026-06-20; extends `crates/cognitive-demo` (the `cognitive-demo` binary) with a finite, enum-backed set of NEGATIVE scenarios that prove the BAD paths cannot smuggle authority. Where MTRACE-0/1 prove the good paths preserve the boundary, MTRACE-2 proves invalid variations FAIL CLOSED. Doctrine: **Failure cases attack the boundary. They do not weaken it. Forged authority is rejected. Nothing executes. Nothing becomes evidence. Nothing promotes. Nothing trains.** Three commands: `cognitive-demo failure-cases` lists the 7 cases; `cognitive-demo failure-pack --out DIR` writes `failure-pack.json` + `failure-report.txt` (the rejection record + its rendered report); `cognitive-demo failure-verify --path DIR` re-derives the whole pack and refuses any tamper. **Seven forged cases**, each DETERMINISTICALLY forging a forbidden authority claim onto a canonical artifact and being REFUSED by the EXISTING re-derive-and-byte-compare verifier: `forged-execution` (intent → `executed`), `forged-evidence` (observation → claims evidence authority), `forged-promotion` (`grants_promotion` → true), `forged-training` (`training_justified` → true) — all four refused by `verify_trace_json` (`TraceError::TraceMismatch`); `forged-review` (a `rejected` scenario review → `approved`) refused by `verify_scenario_bundle` (`BundleMismatch("trace.json")`); `forged-report` (the report narrates execution/evidence) refused by `verify_bundle` (`BundleMismatch("report.txt")`); `forged-matrix` (a coverage cell flipped to hide a failed boundary while the summary still claims full coverage) refused by `verify_scenario_matrix` (`MatrixMismatch`). **Proves, not asserts:** `run_failure_case` builds the canonical artifact fresh, applies the forgery to a COPY, and runs the real verifier — the pack records per case `forgery_applied` (the bytes genuinely changed), `injects_forbidden` (the case's SPECIFIC affirmative-authority token was injected, so a benign byte-change cannot masquerade as a forbidden-authority forgery), `rejected` (OBSERVED from the verifier's `Result`, never hardcoded), and the EXACT typed `rejection_reason`. The rejection is STRUCTURAL (re-derive byte-compare), not a prose grep. **Re-derive, never trust:** `verify_failure_pack` re-derives the canonical pack (re-running every forgery) and byte-compares via the shared `compare_bundle` core; `FailurePack`/`FailureRejection`/`FailureSummary` derive `Serialize` but NOT `Deserialize`, so a doctored pack (e.g. one flipping a `rejected` to false to claim a forgery passed) is refused, never parsed back into authority. **No authority leaks:** the forged bytes are NEVER persisted — only the (prose) rejection record is, so neither emitted file contains any affirmative authority JSON; building the pack leaves the frozen canonical trace byte-identical (happy-boundary still == `demo()`), the MTRACE-0 pack and the MTRACE-1 matrix unchanged, and P12 `training_justified=false`. A self-found vacuity hole was folded BEFORE sabotage: the per-case rejection tests proved the forgery changed bytes and was rejected, but a BENIGN byte-change would also satisfy that — closed by adding `injects_forbidden` (each forgery must inject its exact forbidden-authority token), with the gate pinning `all_inject_forbidden` and the count. 12 new tests (the rubric's 10 first-tests + `failure_pack_forgeries_actually_mutate_canonical` + `failure_pack_verify_rejects_tampered_pack`) → 80 unit total (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3 + 12 MTRACE-0 + 12 MTRACE-1 + 12 MTRACE-2), fmt + clippy clean. release_check MTRACE-2 gate: surface signals + the re-derive pin (`compare_bundle(&failure_pack_files()?`) + the anti-vacuity pins (each forgery's verdict is a real `verify_*` call; `forbidden_token` + `injects_forbidden: forged.contains(token)`) + all 12 MTRACE-2 test-name pins + the unit-count pin raised 68→80 + a BINARY smoke (every case recorded; exactly 7 `forgery_applied`/`injects_forbidden`/`rejected` true; `all_forged`/`all_inject_forbidden`/`all_rejected`; no `false` for any of those; a NO-AUTHORITY guard over BOTH pack files; the report records every case REJECTED — never ACCEPTED — with the exact typed reasons; the seven boundary lines verbatim; deterministic; a doctored pack and a missing file both refused; the frozen happy-boundary trace still byte-equal to `trace`). Three live sabotage probes each restored byte-identical (md5, never `git checkout`): (a) make `verify_failure_pack` TRUST the provided pack (skip the byte-comparison) → caught by `failure_pack_verify_rejects_tampered_pack` (exit 101); (b) make a forgery BENIGN (inject a trailing space, not the forbidden token) → still byte-rejected, suite would have passed the old asserts, yet caught by the new `injects_forbidden` assertion (exit 101) — proving the anti-vacuity check is load-bearing; (c) a boundary-line DRIFT (`Forged authority is rejected.`→`...accepted.`) → the crate suite stayed GREEN (80/0) yet the gate failed (exit 1), caught SOLELY by the binary smoke's verbatim boundary loop. A read-only adversarial panel (Workflow, 4 Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under `mktemp`: forgery-fidelity/vacuity, re-derivation/tamper-rejection, no-authority/overclaim, scope/purity/freeze/gate-vacuity) → **0 REAL findings, fully dry on the first round (4/4 lenses ran, none errored), no debris**. Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` + the gate block; NO Cargo.toml change, NO new file, NO new dependency; no frozen crate source touched, tags unmoved (`reading-track-v0.1`@`f6fa55a`, `hypothesis-track-v0.1`@`bb20acf`, `integration-demo-v0.1`@`95b586d`), P12 `training_justified=false`, P13–P15 closed. No LLM, no training, no execution, no evidence promotion, no frozen crate edits. Gate green + byte-silent (0/0B/0B)._
...
## End-to-End Trace CLI / Operator Report (INT-1): make the trace inspectable, add nothing

Date: 2026-06-19. The second integration sprint. INT-0 proved the chain internally; INT-1 makes it usable and
inspectable by a human operator — run one command, get a readable report plus the machine JSON trace — while
adding NO new authority and NO new cognition. It is a thin surface over the EXISTING canonical trace, not a new
capability. Doctrine unchanged: *Reading verifies. Hypothesis proposes. Probe queue classifies. Governance
reviews. Execution intent records. Observation quarantines. Promotion refuses. Nothing becomes evidence. Nothing
trains.*

The `cognitive-demo` binary has three commands: `trace [--out PATH]` writes the canonical `CognitiveTrace` JSON;
`report --trace PATH [--out PATH]` renders a plain operator report; `replay --trace PATH` confirms a
byte-identical reproduction. The report walks each stage (reading → hypothesis → probe queue → review → intent →
observation → promotion) with the ids and integrity hashes needed to audit and replay, prints the nine-line
authority boundary verbatim, and states in plain prose that nothing executed, nothing became evidence, and
training stayed false.

The load-bearing design is the trust boundary. Because `CognitiveTrace` is `Serialize` but NOT `Deserialize`
(the INT-0 encapsulation guarantee), `report` and `replay` never parse a provided file back into authority.
Instead the pure `verify_trace_json(provided)` RE-DERIVES the canonical trace via `CognitiveTrace::demo()` and
compares the provided string BYTE-FOR-BYTE, refusing any difference with `TraceError::TraceMismatch`; the report
is then rendered from the re-derived canonical trace (`to_report()` reads the trace's own private fields). So a
tampered, stale, or foreign `trace.json` can never be laundered into a clean report or a passing replay — both
refuse it. This is the same "re-derive and reject tamper" discipline the reading track uses for `verify`/`replay`,
applied to the integration surface. `to_report()` is pure FORMATTING: it computes no new verdict, calls no frozen
API, and returns no authority object, so the report can never disagree with, or be more permissive than, the
trace it describes — report prose cannot become authority.

Purity is preserved by structure: the new `src/main.rs` is a thin I/O shell and is the ONLY place `std::fs`
lives; the trace core (`lib.rs`) and the example stay filesystem-free, so the trace RESULT can never depend on
disk — `main.rs` only serializes bytes out and reads bytes in to compare. The CLI spawns no process, opens no
socket, and consults no clock or entropy.

Verification matched the cadence. 8 new tests (the rubric's eight first-tests) bring the crate to 20 unit tests
(12 INT-0 + 8 INT-1), fmt + clippy clean. The `release_check.sh` INT-1 block is load-bearing: the CLI-core and
report signals, the trust-boundary greps (re-derive via `CognitiveTrace::demo()`, refuse via `TraceMismatch`),
all eight INT-1 test-name pins, the unit-count pin raised 12→20, a relaxed-but-stronger purity model (executor
and network banned everywhere in src + example; clock/entropy banned in src; floats banned; and a new fs-confined
scan asserting filesystem calls appear in NEITHER lib.rs NOR the example — only `main.rs`), and an end-to-end
binary smoke that builds the CLI and drives all three commands against temp files: `trace` is byte-identical
across two runs and carries the canonical markers, `report` shows every stage and all nine boundary lines and the
explicit refusals and never an `executed`/`promoted`/`granted` status, `replay` accepts the canonical trace, and
a tampered trace is REFUSED by both replay and report. Three live sabotage probes proved distinct mechanisms:
defeating the `verify_trace_json` comparison so a tampered trace is accepted (caught by the tamper-rejection unit
tests and the binary smoke), leaking `std::fs` into the pure lib core (caught in isolation by the fs-confined
scan — it compiled and passed tests/clippy), and drifting a boundary line (caught by the boundary-lines test and
the binary smoke) — each restored byte-identical by md5, never by `git checkout`. A read-only adversarial panel
(four Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under a temp dir)
returned ZERO real findings on the first round and left no debris; every lens independently confirmed that
tampered and forged traces are refused by both replay and report.

Boundary held: INT-1 adds no authority and no cognition. It serves the SAME canonical trace INT-0 already built;
execution stays `requires_operator` (never executed), promotion stays `rejected` (`grants_promotion=false`), the
observation stays quarantined (`observation_only`), and training stays false and unmoved. Purely additive: only
`crates/cognitive-demo/{Cargo.toml,src/lib.rs}`, the new `src/main.rs`, and the gate block change; no frozen
crate source is touched, the `reading-track-v0.1` (`f6fa55a`) and `hypothesis-track-v0.1` (`bb20acf`) tags are
unmoved, no dependency is added, and P13–P15 stay closed. `release_check.sh` is exit 0 and byte-silent.

## End-to-End Trace Question Harness / Operator Interrogation Surface (INT-2): ask the trace, create no authority

Date: 2026-06-19. The third integration sprint. INT-0 built one canonical trace; INT-1 made it inspectable (a
report). INT-2 makes it INTERROGABLE: an operator can ask fixed, machine-checkable questions about what the trace
did and did not do — without an LLM, without a natural-language parser, and without letting a question or its
answer become authority. Doctrine: *Trace questions explain the trace. They do not create authority. They do not
execute. They do not promote. They do not train.*

The `cognitive-demo` binary gains two commands. `questions` lists the finite, enumerated audit-question set;
`ask --trace PATH --question SLUG [--out PATH]` answers exactly one of them. The eight questions — what-read,
what-was-proven, what-was-hypothesized, what-probe-was-requested, was-anything-executed,
did-anything-become-evidence, why-was-promotion-refused, did-training-open — are the operator's typed lens on the
chain: what was proven (only the reading receipt), what was merely proposed (the hypothesis), whether anything
ran (no), whether anything became evidence (no), and why the promotion was refused.

This is NOT chatbot Q&A; it is a closed interrogation harness. A question is a `TraceQuestion` enum variant, not
a string the model interprets. `TraceQuestion::from_slug` does EXACT-match only — no fuzzy, prefix, case-folding,
or trimming — and returns `None` for anything not in `ALL: [TraceQuestion; 8]`. `run_ask` fails closed TWICE and
in order: first it parses the slug (an unknown slug is `TraceError::UnknownQuestion`, refused WITHOUT consulting
any trace — prose can never become a question), then it re-derives and verifies the trace before any answer is
produced. There is no free-form path into the surface.

The trust boundary is INT-1's, applied to `ask`. Because `CognitiveTrace` is `Serialize` but NOT `Deserialize`, a
provided trace is never parsed back into authority. `run_ask` answers ONLY the trace returned by
`verify_trace_json`, which RE-DERIVES the canonical trace via the pure `CognitiveTrace::demo()` and byte-compares
the provided file, refusing any tampered, stale, or foreign input with `TraceError::TraceMismatch` BEFORE the
answer function runs. So a forged `trace.json` can never be laundered into an answer — a tampered trace is refused
for every question. (A soundness corollary: because only the canonical trace can ever reach the answer renderers,
their fixed yes/no headers describe the only trace that can be answered.)

An answer is prose ABOUT the trace, not new authority. The private `CognitiveTrace::answer` and its eight
`answer_*` renderers FORMAT only the trace's already-recorded private fields (the same private access the report
uses); they compute no new verdict, call no frozen API, and return no authority object — so an answer can never
disagree with, or be more permissive than, the trace. Each answer keeps the distinctions that matter:
`what-was-proven` states that EXACTLY the reading receipt was verified and that nothing downstream is proof (the
hypothesis only proposes, the review only decides, the observation is quarantined, the promotion was refused);
`was-anything-executed` answers No and shows the approved review yielding a `requires_operator` intent, never
`executed`; `did-anything-become-evidence` answers No with the observation quarantined and the promotion
`rejected` (`grants_promotion=false`); `why-was-promotion-refused` explains the refusal and states the promotion
did not occur; `did-training-open` answers No with `training_justified=false`. Every answer ends with the
five-line INT-2 boundary. Purity is unchanged: the whole surface is in the pure library; the only filesystem
access is the pre-existing `main.rs` I/O shell, so there is no new dependency, no new file, and no Cargo.toml
change.

Verification matched the cadence. 12 new tests (the rubric's ten first-tests plus
`ask_answer_preserves_authority_boundary` and `ask_answer_is_not_authority`) bring the crate to 32 unit tests (12
INT-0 + 8 INT-1 + 12 INT-2), fmt + clippy clean. The `release_check.sh` INT-2 block is load-bearing: the surface
signals, the fail-closed/re-derive pins (`TraceQuestion::from_slug`, `verify_trace_json`, `UnknownQuestion`), all
twelve INT-2 test-name pins, the unit-count pin raised 20→32, and an end-to-end binary smoke that drives
`questions` and `ask` against real files — `questions` lists all eight slugs, `ask what-read` carries the trace's
real `reading_answer_hash` and all five boundary lines verbatim, the no/refusal answers say so explicitly, a
no-authority guard asserts no answer line ends in an affirmative `executed`/`promoted`/`granted`/`recorded` status
and no grant reads `true`, and an unknown question and a tampered trace are BOTH refused (exit non-zero). One
self-found gap was folded before sabotage: the boundary smoke originally pinned only two of the five lines and the
test pinned only lines [0]/[4] as literals, so a drift in a middle line ("create authority", "execute", "promote")
could slip both — closed by adding a verbatim five-line loop to the gate and pinning all five literals in the
test. Three live sabotage probes proved distinct mechanisms, each restored byte-identical by md5 (never `git
checkout`): (a) fail-OPEN the ask tamper-refusal so a tampered trace is answered with canonical content — caught
by `ask_refuses_tampered_trace` (exit 101) and the binary smoke; (b) fail-OPEN an unknown question by defaulting
to a variant — caught by `ask_refuses_unknown_question` (exit 101) and the binary smoke; (c) a COORDINATED
boundary drift ("They do not promote."→"They may promote." in BOTH the const AND its test literal) that kept the
crate test suite GREEN (32/0) yet still failed the gate (exit 1), caught SOLELY by the new verbatim five-line loop
— the strongest demonstration that the hardening is load-bearing even against a matching test edit. A read-only
adversarial panel (Workflow, four Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled
binary under a temp dir: fail-closed/closure, trust-boundary/tamper-laundering, answer-fidelity/no-authority/
overclaim, scope/no-frozen-edit/purity/gate-vacuity) returned ZERO real findings on the first round and left no
debris; every lens independently drove the binary (seven-to-eight non-slug questions refused, four-to-six
tampered/foreign traces refused before answering, all eight answers carrying the canonical hashes, five boundary
lines, and no affirmative status).

Boundary held: INT-2 adds no authority and no cognition. It serves the SAME canonical trace INT-0 built and INT-1
reports; execution stays `requires_operator` (never executed), the promotion stays `rejected`
(`grants_promotion=false`), the observation stays quarantined (`observation_only`), and training stays false and
unmoved. Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` and the gate block change; no
Cargo.toml change, no new file, no new dependency; no frozen crate source is touched, the `reading-track-v0.1`
(`f6fa55a`) and `hypothesis-track-v0.1` (`bb20acf`) tags are unmoved, and P13–P15 stay closed. `release_check.sh`
is exit 0 and byte-silent.

## Prototype Demo Bundle / Operator Repro Pack (INT-3): one reproducible pack, re-derived not trusted

Date: 2026-06-19. The fourth integration sprint. INT-0 built one canonical trace; INT-1 made it inspectable (a
report); INT-2 made it interrogable (the question harness). INT-3 makes it PORTABLE: one command produces a
reproducible operator pack that shows what the prototype can do, and a second command verifies that pack by
re-deriving it — so the files are a demonstration, never authority. Doctrine: *The bundle demonstrates the
prototype. It does not create evidence. It does not create authority. It does not execute. It does not promote.
It does not train.*

The `cognitive-demo` binary gains two commands. `bundle --out DIR` writes four files — `trace.json` (the
canonical machine record), `report.txt` (the operator report), `questions.txt` (the full audit transcript: the
`questions` menu followed by every enumerated question and its answer), and `manifest.json` (a manifest that
hashes every content file, plus a replay proof and the six-line boundary). `bundle-verify --path DIR` reads the
pack back and checks it. All four files are PURELY derived from the canonical trace, so the whole pack is a pure
function of fixed inputs: two bundles are byte-identical, file for file.

The load-bearing design is the same re-derivation trust boundary the prototype has held since INT-1, now applied
to a multi-file pack. `bundle-verify` does NOT trust the files. `verify_bundle` re-derives the canonical bundle
via `canonical_bundle()` (which builds from `run_trace` / `CognitiveTrace::demo()`) and byte-compares each
provided file against the freshly re-derived canonical; a missing file is `TraceError::BundleMissingFile` and any
tampered, stale, or foreign file — including the manifest — is `TraceError::BundleMismatch`. It never parses or
deserializes a provided file into trusted state, and it never checks the manifest's own recorded hash against the
file (that would be trusting the file). `CognitiveTrace` and the new `BundleManifest`/`BundleFileEntry`/
`BundleReplayProof` all derive `Serialize` but NOT `Deserialize`, so no bundle file can be read back into
authority — a tampered bundle can never pass, and the manifest can never vouch for a forged pack because the
manifest is itself re-derived and byte-compared.

The manifest is honest. `bundle_content_hash` is Rust's `DefaultHasher` (deterministic and dependency-free),
named in the manifest as `rust-default-hasher-u64-hex` — NOT claimed as a cryptographic digest. It hashes the
three content files with distinct, content-dependent hashes and does NOT hash itself (no fixpoint). The hash is a
demonstrable digest; the load-bearing integrity check is the full byte-for-byte re-derivation of every file, of
which the hash is only a visible part. Purity is preserved by structure: the filesystem reads and writes
(`write_bundle`/`read_bundle`) live ONLY in `src/main.rs`; the library that derives and verifies the bundle is
filesystem-free, so the bundle CONTENT can never depend on disk. The bundle creates no authority and no evidence
— no file shows an affirmative `executed`/`promoted`/`granted`/`recorded` status or a true grant, the trace
records `training_justified=false`, and the P12 verdict and verifier receipt are left unmoved.

Verification matched the cadence. 12 new tests (the rubric's ten first-tests plus
`bundle_verify_rejects_tampered_manifest` and `bundle_output_is_not_authority`) bring the crate to 44 unit tests
(12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3), fmt + clippy clean. The `release_check.sh` INT-3 block is
load-bearing: the surface signals, the re-derive pin (`let canonical = canonical_bundle()`), all twelve INT-3
test-name pins, the unit-count pin raised 32→44, and an end-to-end binary smoke that writes a real pack and
proves the four files are present, the manifest names and hashes every content file, the six boundary lines are
present verbatim, two bundles are byte-identical file-for-file, no content file shows an affirmative status, and
`bundle-verify` accepts the pristine pack while refusing a tamper of EACH file (trace/report/questions/manifest),
a missing file, and a foreign bundle. One self-found gap was folded before sabotage: `bundle_manifest_hashes_all_files`
recomputes hashes with the same `bundle_content_hash`, so a constant/fake hash would slip both it and the
`content_hash` count check — closed by adding a gate check that the three content hashes are DISTINCT
(content-dependent). Three live sabotage probes proved distinct mechanisms, each restored byte-identical by md5
(never `git checkout`): (a) make `verify_bundle` TRUST the provided files (skip the byte-comparison) — caught by
all four tamper tests (exit 101) and the binary smoke; (b) a constant fake `bundle_content_hash` that kept the
crate suite GREEN (44/0, because the hash test is self-referential) yet still failed the gate (exit 1), caught
SOLELY by the new distinct-hash check; (c) a COORDINATED boundary drift ("It does not promote."→"It may promote."
in BOTH the const AND its test literal) that kept the suite GREEN (44/0) yet failed the gate (exit 1), caught
SOLELY by the binary smoke's verbatim six-line manifest loop — (b) and (c) demonstrate the two hardening checks
are load-bearing even against a matching test edit. A read-only adversarial panel (Workflow, four Explore lenses,
refute-by-default, no-compile-to-disk, each driving the compiled binary under a temp dir: re-derivation/tamper-
rejection, manifest-integrity/no-self-trust, output-not-authority/overclaim, scope/purity/fs-confinement/gate-
vacuity) returned ZERO real findings on the first round and left no debris; every lens independently drove the
binary and confirmed that tampered/missing/foreign bundles are refused, the pristine pack verifies, two bundles
are byte-identical, and no bundle file overclaims.

Boundary held: INT-3 adds no authority and no cognition. It packages the SAME canonical trace INT-0 built, INT-1
reports, and INT-2 interrogates; execution stays `requires_operator` (never executed), the promotion stays
`rejected` (`grants_promotion=false`), the observation stays quarantined (`observation_only`), and training stays
false and unmoved. Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` and the gate block change;
no Cargo.toml change, no new file, no new dependency; no frozen crate source is touched, the `reading-track-v0.1`
(`f6fa55a`) and `hypothesis-track-v0.1` (`bb20acf`) tags are unmoved, and P13–P15 stay closed. `release_check.sh`
is exit 0 and byte-silent.

## Multi-Trace Scenario Pack / Variation Without Authority Expansion (MTRACE-0): several paths, one boundary

Date: 2026-06-19. The first sprint after the integration-demo-v0.1 freeze. The frozen demo proves ONE canonical
path; MTRACE-0 proves the SAME authority boundary across several deterministic scenarios — without adding any
capability or model behavior. Doctrine: *Scenarios vary the path. They do not vary the authority. Nothing
executes. Nothing becomes evidence. Nothing promotes. Nothing trains.*

The `cognitive-demo` binary gains three commands. `scenarios` lists the finite scenario set; `scenario-pack
--out DIR` writes one bundle subdirectory per scenario (each the same four files INT-3 defined) plus a
`pack-manifest.json` recording every scenario; `scenario-verify --path DIR` re-derives the whole pack and refuses
any tamper. The four scenarios are deterministic outcomes of the FROZEN hypothesis chain: `happy-boundary`
(governance approves a low-risk, reversible queued probe → intent `requires_operator`, observation
`requires_review`, promotion `rejected`); `review-rejected` (governance rejects a queued probe → intent `blocked`,
observation `rejected`); `review-deferred` (governance defers → intent `blocked`, observation `rejected`); and
`high-risk-blocked` (the probe is classified `blocked` because it is high-risk AND irreversible, so there is no
approval path — the frozen layer refuses to approve a blocked probe — and nothing can execute). The rubric's
`tampered-input` is not a path variant; it is the cross-cutting verify-refusal property, tested for every
scenario.

The point is variation WITHOUT authority expansion. A `Scenario` enum varies only the probe's risk and
reversibility and the governance decision — `Scenario::risk`/`reversibility`/`review_decision` are passed into the
new `CognitiveTrace::build_scenario`, and `build()` now simply delegates to it with `HappyBoundary`. Everything
else is identical and read from the frozen crates: the reading verification, the receipt citation, the chain
linkage, and the verdict computation. So every scenario preserves the entire boundary: the execution status is
never `executed` (`nothing_executed`), the observation is never `recorded` and stays `observation_only`
(`observation_quarantined`), the promotion is `rejected` with `grants_promotion=false`
(`promotion_refused`/`nothing_becomes_evidence`), and `training_justified` stays `false` with the verifier receipt
byte-identical. A rejected or deferred review yields a `blocked` (never executable) intent — the frozen
`from_review` maps Rejected/Deferred to Blocked — and a blocked probe has no approval path. The scenarios are
genuinely distinguishable (different review decisions, statuses, and ids), but the authority is identical.

Verification by re-derivation extends to the pack. `scenario_bundle` and `scenario_pack_manifest` are pure
derivations; `verify_scenario_bundle` and `verify_scenario_pack_manifest` re-derive the canonical scenario bundle
and pack manifest and byte-compare via the shared `compare_bundle` core — `CognitiveTrace`, `BundleManifest`, and
the new `ScenarioPackManifest` all derive `Serialize` but NOT `Deserialize`, so no file is parsed back into
authority and a tampered, missing, or foreign scenario is refused. The library stays pure: the scenario builders
touch no filesystem; the only `std::fs` is the `main.rs` shell, so the pack is a pure function of fixed inputs
(two packs are byte-identical, file for file).

The load-bearing risk of this sprint was a freeze regression: parameterizing `build()` (and making
`canonical_bundle`/`run_questions_doc`/`verify_bundle` delegate to shared cores `trace_bundle`/`questions_doc`/
`compare_bundle`/`bundle_manifest_with`) could have drifted the frozen integration-demo-v0.1 canonical trace. It
did not — all 44 frozen tests pass, and the happy-boundary scenario is byte-identical to `CognitiveTrace::demo()`.
One self-found gap was folded before sabotage: `happy_boundary_scenario_equals_canonical_demo` compares
`scenario_trace(Happy)` to `demo()`, but both use the SAME parameterized builder, so a silent drift of the
happy-boundary risk/reversibility (which leaves the path and statuses unchanged) would slip it AND the status
greps — closed by pinning the frozen canonical `hypothesis_id` (`16880898425785712701`, a stable FNV id from the
frozen hypothesis-layer, not a `DefaultHasher` value) literally in the gate.

Verification matched the cadence. 12 new tests (the rubric's ten first-tests plus
`happy_boundary_scenario_equals_canonical_demo` and `scenarios_are_distinguishable`) bring the crate to 56 unit
tests (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3 + 12 MTRACE-0), fmt + clippy clean. The `release_check.sh`
MTRACE-0 block is load-bearing: the surface signals, the re-derive pins, all twelve MTRACE-0 test-name pins, the
unit-count pin raised 44→56, the `hypothesis_id` freeze-pin, and a binary smoke that writes the pack (four
subdirectories of four files plus the pack manifest), proves it byte-identical across two runs, confirms the
scenarios are distinguishable by status (a `requires_operator` and a `blocked` execution status, a `blocked`
probe), runs a no-authority guard over the whole pack, and confirms `scenario-verify` accepts the pristine pack
and refuses a tampered scenario trace, a tampered scenario manifest, a tampered pack manifest, a missing file, and
a foreign scenario. Three live sabotage probes proved distinct mechanisms, each restored byte-identical by md5
(never `git checkout`): (a) making a rejected review APPROVE (a rejected review producing executable intent) —
caught by `review_rejected_scenario_blocks_intent` (exit 101) and the binary smoke; (b) making
`verify_scenario_bundle` TRUST the provided files (skipping the byte-comparison) — caught by
`tampered_scenario_bundle_is_refused` (exit 101) and the smoke; (c) a silent happy-boundary canonical drift
(`Scenario::risk(HappyBoundary)` 100→200, leaving the path unchanged) that kept the crate suite GREEN (56/0) yet
still failed the gate (exit 1), caught SOLELY by the `hypothesis_id` freeze-pin — demonstrating that the
freeze-preservation pin is load-bearing. A read-only adversarial panel (Workflow, four Explore lenses,
refute-by-default, no-compile-to-disk, each driving the compiled binary under a temp dir:
scenario-correctness/boundary-preservation, freeze-preservation/happy==canonical, re-derivation/tamper-rejection,
scope/purity/gate-vacuity) returned ZERO real findings on the first round and left no debris; every lens
independently drove the binary and confirmed that all four scenarios hold the boundary, the happy-boundary trace
is byte-identical to the canonical demo, all five tamper/missing/foreign cases are refused, and the pack is
deterministic.

Boundary held: MTRACE-0 adds no authority and no cognition. It runs the SAME frozen chain under different
review/observation/promotion outcomes, and every path keeps execution non-running, the observation quarantined,
the promotion refused, and training false and unmoved. Purely additive: only `crates/cognitive-demo/src/{lib.rs,
main.rs}` and the gate block change; no Cargo.toml change, no new file, no new dependency; no frozen crate source
is touched, the `reading-track-v0.1` (`f6fa55a`), `hypothesis-track-v0.1` (`bb20acf`), and `integration-demo-v0.1`
(`95b586d`) tags are unmoved, and P13–P15 stay closed. `release_check.sh` is exit 0 and byte-silent.

## Scenario Matrix / Boundary Coverage Report (MTRACE-1): the coverage view, re-derived not trusted

Date: 2026-06-20. MTRACE-0 produces several scenario bundles; MTRACE-1 summarizes them into a deterministic,
machine-checkable coverage matrix so an operator can see, at a glance, which authority boundaries were proven
across which paths. Doctrine: *The matrix summarizes coverage. It does not create authority. It does not execute.
It does not promote. It does not train.*

The `cognitive-demo` binary gains three commands. `scenario-matrix --pack DIR --out matrix.json` emits the
canonical coverage matrix; `scenario-matrix-report --matrix matrix.json --out matrix.txt` renders a plain report;
`scenario-matrix-verify --pack DIR --matrix matrix.json` re-derives and checks both the pack and the matrix. The
matrix has one row per scenario recording its PATH (review, probe, intent, observation, promotion status, and the
`training_not_justified` verdict) and, for every scenario, the four BOUNDARY cells `no_execution`, `no_evidence`,
`no_promotion`, `no_training` — all true — plus a coverage summary: four scenarios × four boundaries = sixteen
cells, all proven, `all_boundaries_hold=true`, and the distinct review/intent/probe statuses that prove the
variation is real (a `requires_operator` and a `blocked` intent; a `queued` and a `blocked` probe; three review
decisions).

The matrix PROVES rather than asserts: every boundary cell is the trace's real verdict
(`no_execution=trace.nothing_executed()`, and so on), and every status row matches the scenario's trace exactly.
The load-bearing discipline is the one the prototype has held since INT-1, now applied to the coverage view:
re-derive, never trust. The matrix is purely re-derived from `Scenario::ALL` and never reads the pack files for
its content; `scenario-matrix --pack` first VERIFIES the pack — re-deriving every scenario bundle and the pack
manifest through the new pure `verify_scenario_pack` — and refuses a tampered pack before emitting. `verify_scenario_matrix`
and `scenario_matrix_report` re-derive the canonical matrix and byte-compare the provided JSON, returning
`TraceError::MatrixMismatch` on any difference; the report is rendered from the re-derived canonical struct, never
the provided file. `ScenarioMatrix`, `MatrixRow`, and `MatrixCoverage` derive `Serialize` but NOT `Deserialize`,
so a provided matrix is never parsed back into authority — a tampered matrix OR a tampered pack is refused by
verify, report, and emit alike. No matrix or report field shows an affirmative executed/promoted/granted/recorded
status, a true grant, or a `training_justified` verdict; the report is pure formatting of the re-derived matrix.
The frozen canonical trace is unchanged (happy-boundary still equals `demo()`), and `scenario-verify` was
refactored onto the shared `verify_pack_at`→`verify_scenario_pack` core with its behaviour preserved.

Verification matched the cadence. 12 new tests (the rubric's ten first-tests plus
`scenario_matrix_distinguishes_all_paths` and `scenario_matrix_report_is_not_authority`) bring the crate to 68
unit tests (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3 + 12 MTRACE-0 + 12 MTRACE-1), fmt + clippy clean. The
`release_check.sh` MTRACE-1 block is load-bearing: the surface signals, the re-derive pin, all twelve test-name
pins, the unit-count pin raised 56→68, and a binary smoke that emits the matrix and proves it records every
scenario with all status fields and the boundary cells, `cells_proven=16` with `all_boundaries_hold=true`, no
false boundary cell or `training_justified` verdict, distinguishable paths, a no-authority guard, two matrices
byte-identical, the report carrying the coverage and the five boundary lines verbatim plus the explicit
no-execution prose, and `scenario-matrix-verify` accepting the pristine pack+matrix while refusing a tampered
matrix (by verify AND report) and a tampered pack (by emit AND verify). Three live sabotage probes proved distinct
mechanisms, each restored byte-identical by md5 (never `git checkout`): (a) making `verify_scenario_matrix` TRUST
the provided matrix (skipping the byte-comparison) — caught by `scenario_matrix_verify_rejects_tampered_matrix`
(exit 101) and the smoke; (b) making `verify_scenario_pack` SKIP the per-scenario bundle verification — caught by
`scenario_matrix_verify_rejects_tampered_pack` (exit 101) and the smoke; (c) a matrix boundary-line drift that
kept the crate suite GREEN (68/0, because the report-boundary test iterates the const) yet still failed the gate
(exit 1), caught SOLELY by the binary smoke's verbatim report-boundary loop. A read-only adversarial panel
(Workflow, four Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under a
temp dir: matrix-coverage/fidelity, re-derivation/tamper-rejection, output-not-authority/no-overclaim,
scope/purity/freeze/gate-vacuity) returned ZERO real findings and left no debris. The first panel attempt was
abandoned mid-run by a session-usage limit — treated as ABSENCE of verification, not a pass — and the panel was
re-run to a genuine dry round before close; every lens then independently drove the binary and confirmed each row
matches its trace, all boundaries hold, the tampered matrix and pack are refused by verify/report/emit, and the
canonical trace is byte-identical.

Boundary held: MTRACE-1 adds no authority and no cognition. It is a coverage VIEW of the scenario pack — output,
not authority — and every scenario it summarizes keeps execution non-running, the observation quarantined, the
promotion refused, and training false. Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` and the
gate block change; no Cargo.toml change, no new file, no new dependency; no frozen crate source is touched, the
`reading-track-v0.1` (`f6fa55a`), `hypothesis-track-v0.1` (`bb20acf`), and `integration-demo-v0.1` (`95b586d`)
tags are unmoved, and P13–P15 stay closed. `release_check.sh` is exit 0 and byte-silent.

## Scenario Failure Injection / Boundary Regression Pack (MTRACE-2): the bad paths fail closed

Date: 2026-06-20. MTRACE-1 proves coverage under valid scenario variation; MTRACE-2 is the next pressure test —
not just "good paths preserve authority," but "bad paths cannot smuggle authority." It adds a finite, enum-backed
set of NEGATIVE scenarios, each of which deterministically forges a forbidden authority claim and proves the
existing verifiers reject it. Doctrine: *Failure cases attack the boundary. They do not weaken it. Forged authority
is rejected. Nothing executes. Nothing becomes evidence. Nothing promotes. Nothing trains.*

The `cognitive-demo` binary gains three commands. `failure-cases` lists the seven cases; `failure-pack --out DIR`
writes the rejection record (`failure-pack.json`) and its rendered report (`failure-report.txt`); `failure-verify
--path DIR` re-derives the whole pack and refuses any tamper. Each of the seven `FailureCase` variants forges one
forbidden authority claim onto a canonical artifact and is REFUSED by the EXISTING re-derive-and-byte-compare
verifier — there is no new verification logic, only a curated set of attacks against the verifiers MTRACE-0/1
already established. `forged-execution`, `forged-evidence`, `forged-promotion`, and `forged-training` forge the
trace (intent claims `executed`; observation claims evidence authority; `grants_promotion` true; `training_justified`
true) and are refused by `verify_trace_json` (`TraceMismatch`). `forged-review` forges a `rejected` scenario
review to `approved` and is refused by `verify_scenario_bundle` (`BundleMismatch` on `trace.json`).
`forged-report` forges the operator report to narrate that execution and evidence occurred and is refused by
`verify_bundle` (`BundleMismatch` on `report.txt`). `forged-matrix` flips a coverage cell to hide a failed
boundary while the summary still claims full coverage and is refused by `verify_scenario_matrix` (`MatrixMismatch`).
Four surfaces, one discipline.

The pack PROVES rather than asserts. `run_failure_case` builds the canonical artifact fresh, applies the forgery to
a COPY (never mutating any canonical data), and runs the real verifier; it records `forgery_applied` (the bytes
genuinely changed), `injects_forbidden` (the case's specific affirmative-authority token was injected), `rejected`
(observed from the verifier's `Result`, never hardcoded), and the exact typed `rejection_reason`. The rejection is
structural — a re-derive byte-compare against the freshly re-derived canonical — not a prose grep. The load-bearing
discipline is again re-derive, never trust: `verify_failure_pack` re-derives the canonical pack (re-running every
forgery) and byte-compares through the shared `compare_bundle` core, and `FailurePack`, `FailureRejection`, and
`FailureSummary` derive `Serialize` but NOT `Deserialize`, so a doctored pack — one that flips a `rejected` to
false to claim a forgery passed — is refused, never parsed back into authority. Critically, the forged bytes are
NEVER persisted: only the prose rejection record is written, so neither emitted file carries any affirmative
authority JSON. Building the pack leaves the frozen canonical trace byte-identical (happy-boundary still equals
`demo()`), the MTRACE-0 pack and the MTRACE-1 matrix unchanged, and P12 `training_justified=false`.

One vacuity hole was self-found and folded BEFORE sabotage: the per-case rejection tests proved the forgery changed
bytes and was rejected — but a BENIGN byte-change would satisfy that too, since byte-compare rejects any change.
The fix was `injects_forbidden`: each forgery must inject its exact forbidden-authority token (checked against the
forged artifact), so a benign change cannot masquerade as a forbidden-authority forgery; the gate pins
`all_inject_forbidden` and the per-case count.

Verification matched the cadence. 12 new tests (the rubric's ten first-tests plus
`failure_pack_forgeries_actually_mutate_canonical` and `failure_pack_verify_rejects_tampered_pack`) bring the crate
to 80 unit tests (12 INT-0 + 8 INT-1 + 12 INT-2 + 12 INT-3 + 12 MTRACE-0 + 12 MTRACE-1 + 12 MTRACE-2), fmt + clippy
clean. The `release_check.sh` MTRACE-2 block is load-bearing: the surface signals, the re-derive pin
(`compare_bundle(&failure_pack_files()?`), the anti-vacuity pins (each forgery's verdict is a real `verify_*` call;
`forbidden_token` + `injects_forbidden: forged.contains(token)`), all twelve test-name pins, the unit-count pin
raised 68→80, and a binary smoke that emits the pack and proves every case is recorded with exactly seven
`forgery_applied`/`injects_forbidden`/`rejected` true (and `all_forged`/`all_inject_forbidden`/`all_rejected`), no
`false` among them, a no-authority guard over BOTH pack files, the report recording every case REJECTED — never
ACCEPTED — with the exact typed reasons, the seven boundary lines verbatim, determinism, refusal of a doctored
pack and a missing file, and the frozen happy-boundary trace still byte-equal to `trace`. Three live sabotage
probes proved distinct mechanisms, each restored byte-identical by md5 (never `git checkout`): (a) making
`verify_failure_pack` TRUST the provided pack (skipping the byte-comparison) — caught by
`failure_pack_verify_rejects_tampered_pack` (exit 101); (b) making a forgery BENIGN (injecting a trailing space
instead of the forbidden token) — still byte-rejected, so the old asserts would have passed, yet caught by the new
`injects_forbidden` assertion (exit 101), proving the anti-vacuity check is load-bearing; (c) a boundary-line drift
(`Forged authority is rejected.`→`...accepted.`) that kept the crate suite GREEN (80/0) yet still failed the gate
(exit 1), caught SOLELY by the binary smoke's verbatim boundary loop. A read-only adversarial panel (Workflow, four
Explore lenses, refute-by-default, no-compile-to-disk, each driving the compiled binary under a temp dir:
forgery-fidelity/vacuity, re-derivation/tamper-rejection, no-authority/overclaim, scope/purity/freeze/gate-vacuity)
returned ZERO real findings, fully dry on the first round (4/4 lenses ran, none errored), and left no debris.

Boundary held: MTRACE-2 adds no authority and no cognition. It is an adversarial REGRESSION suite — it forges
forbidden authority and demonstrates the existing verifiers refuse it — and every forgery it records stays
rejected, with execution non-running, the observation quarantined, the promotion refused, and training false.
Purely additive: only `crates/cognitive-demo/src/{lib.rs,main.rs}` and the gate block change; no Cargo.toml change,
no new file, no new dependency; no frozen crate source is touched, the `reading-track-v0.1` (`f6fa55a`),
