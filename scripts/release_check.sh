...
# LEARNER-MEMORY-1: the journal module is the module most tempted to touch persistence, so it gets its own
# purity pins IN ADDITION to the crate-wide scans above (which cover time/entropy/net/floats everywhere but
# scan fs only in lib.rs + examples). The journal LIBRARY stays a pure fold — all journal file I/O lives in
# the main.rs shell — and derives no Deserialize (derive OR hand-written impl): a stored journal is untrusted
# input that is byte-verified against the re-derived canonical state, never parsed.
test -f crates/cognitive-demo/src/learner_journal.rs
test "$(grep -cE 'std::fs|File::create|File::open|fs::write|fs::read|OpenOptions|Command::new|process::Command|std::net|TcpStream|UdpSocket|SystemTime|Instant|std::time|thread_rng|getrandom|rand::|use rand' crates/cognitive-demo/src/learner_journal.rs)" -eq 0
test "$(grep -cE 'derive\([^)]*Deserialize' crates/cognitive-demo/src/learner_journal.rs)" -eq 0
test "$(grep -cE 'impl([[:space:]]|<).*Deserialize.*for' crates/cognitive-demo/src/learner_journal.rs)" -eq 0
# All 5 LEARNER-MEMORY-1 CLI verbs stay wired in the I/O shell (consented append-only journal surface).
grep -q '"learner-journal-demo"' crates/cognitive-demo/src/main.rs
grep -q '"learner-journal-demo-verify"' crates/cognitive-demo/src/main.rs
grep -q '"learner-journal-matrix"' crates/cognitive-demo/src/main.rs
grep -q '"learner-journal-matrix-verify"' crates/cognitive-demo/src/main.rs
grep -q '"learner-journal-append"' crates/cognitive-demo/src/main.rs
# SESSION-LOOP-0: the session composer is a pure fold over the frozen organs — same per-module purity pins
# as the journal (no fs/process/net/time/entropy tokens, no Deserialize derive or hand-written impl), and
# the four session CLI verbs stay wired in the I/O shell.
test -f crates/cognitive-demo/src/learning_session.rs
test "$(grep -cE 'std::fs|File::create|File::open|fs::write|fs::read|OpenOptions|Command::new|process::Command|std::net|TcpStream|UdpSocket|SystemTime|Instant|std::time|thread_rng|getrandom|rand::|use rand' crates/cognitive-demo/src/learning_session.rs)" -eq 0
test "$(grep -cE 'derive\([^)]*Deserialize' crates/cognitive-demo/src/learning_session.rs)" -eq 0
test "$(grep -cE 'impl([[:space:]]|<).*Deserialize.*for' crates/cognitive-demo/src/learning_session.rs)" -eq 0
grep -q '"learning-session-demo"' crates/cognitive-demo/src/main.rs
...
# GAME-EVIDENCE-0: the evidence adapter is a pure fold from typed fixture observations to
# strict verbatim-preserving documents — same per-module purity pins (no fs/process/net/
# time/entropy tokens, no Deserialize derive or hand-written impl), the four game-evidence
# CLI verbs stay wired in the I/O shell, and the modules it feeds stay intact: the arc's
# unit-test count and entry point are pinned exactly like the session/journal pins above
# (touching learning_arc.rs or learning_session.rs trips these without a separately gated
# sprint updating them).
test -f crates/cognitive-demo/src/game_evidence.rs
test "$(grep -cE 'std::fs|File::create|File::open|fs::write|fs::read|OpenOptions|Command::new|process::Command|std::net|TcpStream|UdpSocket|SystemTime|Instant|std::time|thread_rng|getrandom|rand::|use rand' crates/cognitive-demo/src/game_evidence.rs)" -eq 0
test "$(grep -cE 'derive\([^)]*Deserialize' crates/cognitive-demo/src/game_evidence.rs)" -eq 0
test "$(grep -cE 'impl([[:space:]]|<).*Deserialize.*for' crates/cognitive-demo/src/game_evidence.rs)" -eq 0
grep -q '"game-evidence-demo"' crates/cognitive-demo/src/main.rs
grep -q '"game-evidence-demo-verify"' crates/cognitive-demo/src/main.rs
grep -q '"game-evidence-matrix"' crates/cognitive-demo/src/main.rs
grep -q '"game-evidence-matrix-verify"' crates/cognitive-demo/src/main.rs
test "$(grep -c '#\[test\]' crates/cognitive-demo/src/learning_arc.rs)" -eq 17
grep -q 'pub fn run_learning_arc(' crates/cognitive-demo/src/learning_arc.rs
# WOW-STATE-0: the navigation/situation state organ is a pure integer fold from typed fixture
# observations to a deterministic nav snapshot — same per-module purity pins (no fs/process/net/
# time/entropy tokens, no Deserialize derive or hand-written impl), and it is FLOAT-FREE like the
# rest of the crate (distance is an integer isqrt, bearing an integer CORDIC in millidegrees — the
# crate-wide float scan above already fails closed if a float ever appears here). The four
# wow-state CLI verbs stay wired in the I/O shell, and the prior rung it sits above stays intact:
# game_evidence.rs unit-test count and entry point are pinned exactly like the arc/session pins.
test -f crates/cognitive-demo/src/wow_state.rs
test "$(grep -cE 'std::fs|File::create|File::open|fs::write|fs::read|OpenOptions|Command::new|process::Command|std::net|TcpStream|UdpSocket|SystemTime|Instant|std::time|thread_rng|getrandom|rand::|use rand' crates/cognitive-demo/src/wow_state.rs)" -eq 0
test "$(grep -cE 'derive\([^)]*Deserialize' crates/cognitive-demo/src/wow_state.rs)" -eq 0
test "$(grep -cE 'impl([[:space:]]|<).*Deserialize.*for' crates/cognitive-demo/src/wow_state.rs)" -eq 0
grep -q '"wow-state-demo"' crates/cognitive-demo/src/main.rs
grep -q '"wow-state-demo-verify"' crates/cognitive-demo/src/main.rs
grep -q '"wow-state-matrix"' crates/cognitive-demo/src/main.rs
grep -q '"wow-state-matrix-verify"' crates/cognitive-demo/src/main.rs
test "$(grep -c '#\[test\]' crates/cognitive-demo/src/wow_state.rs)" -eq 27
